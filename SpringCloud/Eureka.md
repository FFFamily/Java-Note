...
接下来我们修改consumer，尝试从EurekaServer获取服务。
...
当服务消费者启动时，会检测`fetch-registry`参数的值，如果为true，则会拉取Eureka Server服务的列表只读备份，然后缓存在本地。

```properties
eureka.client.fetch-registry=true
```

并且`每隔5秒`会重新获取并更新数据。我们可以通过下面的参数来修改：
