...
三个重要属性：position（当前位置）、limit（限制）、caoacity（容量）
...
目前流行的多路复用I/O的实现主要包括四种：select、poll、epoll、kqueue。
...
```java
Bytebuffer buf = ByteBuffer.allocate(16);// 固定的，不能动态分配
```

--

```java
Bytebuffer.allocate().getClass() //HeapByteBuffer
Bytebuffer.allocateDirect().getClass() // DirectByteBuffer
```

两者区别

- HeapByteBuffer 使用堆内存，读写效率低，还可能会收到GC垃圾回收的影响‘

> 有些垃圾回收会将垃圾回收后会对内存进行整理

- DirectByteBuffer 使用直接的本地内存，读写效率高，少一次拷贝，但分配内存慢，不会受到GC的影响，使用不当会导致内存泄露
...
```java
byte b = buf.get();
byte b = buf.get(i);// 这样不会改变position的位置
```
...
try (RandomAccessFile file = new RandomAccessFile("helloword/3parts.txt", "rw")) {
    ...
    // 读的ByteBuffer数组，可以读多个ByteBuffer
...
//                }
                ...
                while (iter.hasNext()) {
                    ...
                    if (key.isAcceptable()) {
                        ...
                        // 处理read问题时，会遇到客户端断开连接导致IO异常
                        int read = sc.read(buffer);// 正常断开 返回值是 -1
                        if(read == -1) {
                            key.cancel();
                            sc.close();
                        } else {
        ...
        while(true) {
            ...
            while (iter.hasNext()) {
                ...
                if (key.isAcceptable()) {
                    ...
                    ByteBuffer buffer = (ByteBuffer) key.attachment();
                    SocketChannel sc = (SocketChannel) key.channel();
                    // 有一个缓冲区的概念，也就是缓冲区的写满后，这里就不会发数据，会等待缓冲区
                    int write = sc.write(buffer);
                    System.out.println("实际写入字节:" + write);
